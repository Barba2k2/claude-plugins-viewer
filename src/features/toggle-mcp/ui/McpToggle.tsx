...
import { toggleMcp } from '@/features/toggle-mcp/api/mcps';
import { useToggleStore } from '@/features/toggle-resource/model/toggleStore';
...
export function McpToggle({ name, enabled, size = 'sm' }: Props) {
  const key = `mcp:${name}`;
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    startTransition(async () => {
      const result = await toggleMcp(name, next);
      if (!result.success) {
        setOptimistic(key, !next);
  ...
  return (
    <button
      ...
      onClick={handleClick}
      disabled={pending}
      title={error ?? (optimistic ? 'Disable MCP' : 'Enable MCP')}
      className={`relative inline-flex ${trackW} ${trackH} shrink-0 items-center rounded-full border border-border transition ${
        optimistic ? 'bg-accent/30' : 'bg-bg'
      } ${pending ? 'opacity-50' : ''}`}
      aria-label={optimistic ? 'Disable MCP' : 'Enable MCP'}
